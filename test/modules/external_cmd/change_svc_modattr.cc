...
/**
 *  Run change_svc_modattr test.
 */
static void check_change_svc_modattr() {
  ...
  char const* cmd("[1317196300] CHANGE_SVC_MODATTR;name;description;42");
  process_external_command(cmd);

  if (svc->modified_attributes != 42)
    throw (engine_error() << "change_svc_modattr failed.");
...
/**
 *  Check processing of change_svc_modattr works.
 */
int main_test() {
  logging::engine& engine = logging::engine::instance();
  check_change_svc_modattr();
  engine.cleanup();
  return (0);
}
