...
/**
 *  Run disable_servicegroup_host_checks test.
 */
static void check_disable_servicegroup_host_checks() {
  ...
  char const* cmd("[1317196300] DISABLE_SERVICEGROUP_HOST_CHECKS;group");
  process_external_command(cmd);

  if (hst->checks_enabled)
    throw (engine_error() << "disable_servicegroup_host_checks failed.");
...
/**
 *  Check processing of disable_servicegroup_host_checks works.
 */
int main_test() {
  logging::engine& engine = logging::engine::instance();
  check_disable_servicegroup_host_checks();
  engine.cleanup();
  return (0);
}
