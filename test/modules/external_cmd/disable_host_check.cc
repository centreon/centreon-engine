...
/**
 *  Run disable_host_check test.
 */
static void check_disable_host_check() {
  ...
  char const* cmd("[1317196300] DISABLE_HOST_CHECK;name");
  process_external_command(cmd);

  if (hst->checks_enabled)
    throw (engine_error() << "disable_host_check failed.");
...
/**
 *  Check processing of disable_host_check works.
 */
int main(void) {
  try {
    check_disable_host_check();
  }
