...
/**
 *  Run disable_svc_event_handler test.
 */
static void check_disable_svc_event_handler() {
  ...
  char const* cmd("[1317196300] DISABLE_SVC_EVENT_HANDLER;name;description");
  process_external_command(cmd);

  if (svc->event_handler_enabled)
    throw (engine_error() << "disable_svc_event_handler failed.");
...
/**
 *  Check processing of disable_svc_event_handler works.
 */
int main(void) {
  try {
    check_disable_svc_event_handler();
  }
