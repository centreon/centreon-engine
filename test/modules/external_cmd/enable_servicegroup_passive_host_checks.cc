...
/**
 *  Run enable_servicegroup_passive_host_checks test.
 */
static void check_enable_servicegroup_passive_host_checks() {
  ...
  char const* cmd("[1317196300] ENABLE_SERVICEGROUP_PASSIVE_HOST_CHECKS;group");
  process_external_command(cmd);

  if (!hst->accept_passive_host_checks)
    throw (engine_error() << "enable_servicegroup_passive_host_checks failed.");
...
/**
 *  Check processing of enable_servicegroup_passive_host_checks works.
 */
int main(int argc, char** argv) {
  QCoreApplication app(argc, argv);
  try {
    logging::engine& engine = logging::engine::instance();
    check_enable_servicegroup_passive_host_checks();
    engine.cleanup();
  }
