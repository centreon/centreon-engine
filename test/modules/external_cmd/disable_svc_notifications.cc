...
/**
 *  Run disable_svc_notifications test.
 */
static void check_disable_svc_notifications() {
  ...
  char const* cmd("[1317196300] DISABLE_SVC_NOTIFICATIONS;name;description");
  process_external_command(cmd);

  if (svc->notifications_enabled)
    throw (engine_error() << "disable_svc_notifications failed.");
...
/**
 *  Check processing of disable_svc_notifications works.
 */
int main(int argc, char** argv) {
  QCoreApplication app(argc, argv);
  try {
    logging::engine& engine = logging::engine::instance();
    check_disable_svc_notifications();
    engine.cleanup();
  }
