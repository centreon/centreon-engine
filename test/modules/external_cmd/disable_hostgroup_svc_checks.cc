...
/**
 *  Run disable_hostgroup_svc_checks test.
 */
static void check_disable_hostgroup_svc_checks() {
  ...
  char const* cmd("[1317196300] DISABLE_HOSTGROUP_SVC_CHECKS;group");
  process_external_command(cmd);

  if (svc->checks_enabled)
    throw (engine_error() << "disable_hostgroup_svc_checks failed.");
...
/**
 *  Check processing of disable_hostgroup_svc_checks works.
 */
int main(int argc, char** argv) {
  ...
  try {
    ...
    check_disable_hostgroup_svc_checks();
    engine.cleanup();
  }
