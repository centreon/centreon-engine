...
/**
 *  Run disable_host_svc_notifications test.
 */
static void check_disable_host_svc_notifications() {
  ...
  char const* cmd("[1317196300] DISABLE_HOST_SVC_NOTIFICATIONS;name");
  process_external_command(cmd);

  if (svc->notifications_enabled)
    throw (engine_error() << "disable_host_svc_notifications failed.");
...
/**
 *  Check processing of disable_host_svc_notifications works.
 */
int main(void) {
  try {
    check_disable_host_svc_notifications();
  }
