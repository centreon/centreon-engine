...
/**
 *  Run enable_servicegroup_host_notifications test.
 */
static void check_enable_servicegroup_host_notifications() {
  ...
  char const* cmd("[1317196300] ENABLE_SERVICEGROUP_HOST_NOTIFICATIONS;group");
  process_external_command(cmd);

  if (!hst->notifications_enabled)
    throw (engine_error() << "enable_servicegroup_host_notifications failed.");
...
/**
 *  Check processing of enable_servicegroup_host_notifications works.
 */
int main(void) {
  try {
    check_enable_servicegroup_host_notifications();
  }
