...
/**
 *  Run enable_passive_svc_checks test.
 */
static void check_enable_passive_svc_checks() {
  ...
  char const* cmd("[1317196300] ENABLE_PASSIVE_SVC_CHECKS;name;description");
  process_external_command(cmd);

  if (!svc->accept_passive_service_checks)
    throw (engine_error() << "enable_passive_svc_checks failed.");
...
/**
 *  Check processing of enable_passive_svc_checks works.
 */
int main_test() {
  logging::engine& engine = logging::engine::instance();
  check_enable_passive_svc_checks();
  engine.cleanup();
  return (0);
}
