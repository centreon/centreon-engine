...
/**
 *  Run disable_hostgroup_passive_host_checks test.
 */
static void check_disable_hostgroup_passive_host_checks() {
  ...
  char const* cmd("[1317196300] DISABLE_HOSTGROUP_PASSIVE_HOST_CHECKS;group");
  process_external_command(cmd);

  if (hst->accept_passive_host_checks)
    throw (engine_error() << "disable_hostgroup_passive_host_checks failed.");
...
/**
 *  Check processing of disable_hostgroup_passive_host_checks works.
 */
int main(int argc, char** argv) {
  ...
  try {
    ...
    check_disable_hostgroup_passive_host_checks();
    engine.cleanup();
  }
