 ...
 */
static void check_disable_servicegroup_passive_host_checks() {
  ...
  char const* cmd("[1317196300] DISABLE_SERVICEGROUP_PASSIVE_HOST_CHECKS;group");
...
/**
 *  Check processing of disable_servicegroup_passive_host_checks works.
 */
int main(int argc, char** argv) {
  QCoreApplication app(argc, argv);
  try {
    logging::engine& engine = logging::engine::instance();
    check_disable_servicegroup_passive_host_checks();
    engine.cleanup();
  }
