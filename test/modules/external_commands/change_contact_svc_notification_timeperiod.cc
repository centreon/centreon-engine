 ...
 */
static int check_change_contact_svc_notification_timeperiod(int argc,
                                                            char** argv) {
  (void)argc;
  (void)argv;
  ...
  char const* cmd(
      "[1317196300] CHANGE_CONTACT_SVC_NOTIFICATION_TIMEPERIOD;name;tperiod");
  process_external_command(cmd);

  if (cntct->service_notification_period_ptr != tperiod)
    throw(
        engine_error() << "change_contact_svc_notification_timeperiod failed.");
  return (0);
}
