 ...
 */
static int check_enable_svc_event_handler(int argc, char** argv) {
  (void)argc;
  (void)argv;
  ...
  char const* cmd("[1317196300] ENABLE_SVC_EVENT_HANDLER;name;description");
  process_external_command(cmd);

  if (!svc->event_handler_enabled)
    throw(engine_error() << "enable_svc_event_handler failed.");
  return (0);
}
...
int main(int argc, char** argv) {
  unittest utest(argc, argv, &check_enable_svc_event_handler);
  return (utest.run());
}
