 ...
 */
static int check_enable_host_svc_notifications(int argc, char** argv) {
  (void)argc;
  (void)argv;
  ...
  char const* cmd("[1317196300] ENABLE_HOST_SVC_NOTIFICATIONS;name");
  process_external_command(cmd);

  if (!svc->notifications_enabled)
    throw(engine_error() << "enable_host_svc_notifications failed.");
  return (0);
}
...
int main(int argc, char** argv) {
  unittest utest(argc, argv, &check_enable_host_svc_notifications);
  return (utest.run());
}
