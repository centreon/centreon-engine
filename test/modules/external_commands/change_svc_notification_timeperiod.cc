 ...
 */
static int check_change_svc_notification_timeperiod(int argc, char** argv) {
  (void)argc;
  (void)argv;
  ...
  char const* cmd(
      "[1317196300] "
      "CHANGE_SVC_NOTIFICATION_TIMEPERIOD;name;description;tperiod");
...
int main(int argc, char** argv) {
  unittest utest(argc, argv, &check_change_svc_notification_timeperiod);
  return (utest.run());
}
