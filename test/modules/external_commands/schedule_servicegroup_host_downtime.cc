 ...
 */
static int check_schedule_servicegroup_host_downtime(
             int argc,
             char** argv) {
  (void)argc;
  (void)argv;
  ...
  // Create membership.
  servicesmember*
    member(add_service_to_servicegroup(group, "name", "description"));
  if (!member)
    throw (engine_error() << "service group membership creation failed");
  member->service_ptr = svc;

  // Send external command.
  char const* cmd("[1317196300] SCHEDULE_SERVICEGROUP_HOST_DOWNTIME;group;1317196300;2000000000;0;0;7200;user;comment");
  process_external_command(cmd);

  // Check.
  if (!scheduled_downtime_list)
    throw (engine_error()
           << "schedule_servicegroup_host_downtime failed");
...
int main(int argc, char** argv) {
  unittest utest(argc, argv, &check_schedule_servicegroup_host_downtime);
  return (utest.run());
}
