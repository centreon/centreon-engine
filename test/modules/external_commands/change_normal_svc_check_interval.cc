 ...
 */
static int check_change_normal_svc_check_interval(int argc, char** argv) {
  (void)argc;
  (void)argv;
  ...
  char const* cmd(
      "[1317196300] CHANGE_NORMAL_SVC_CHECK_INTERVAL;name;description;42");
...
int main(int argc, char** argv) {
  unittest utest(argc, argv, &check_change_normal_svc_check_interval);
  return (utest.run());
}
