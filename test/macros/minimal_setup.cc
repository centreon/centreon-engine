...
void com::centreon::engine::test::minimal_setup() {
  ...
  if (!add_contact_to_host(hst, const_cast<char*>("mycontact")))
    throw(error() << "could not add default contact to default host");
  hst->contacts->contact_ptr = cntct;
  if (!add_contact_to_service(svc, const_cast<char*>("mycontact")))
    throw(error() << "could not add default contact to default service");
  svc->contacts->contact_ptr = cntct;

  // Add command.
  command* cmd(
      add_command(const_cast<char*>("mycommand"), const_cast<char*>("true")));
  if (!cmd)
    throw(error() << "default command creation failed");
  if (!add_host_notification_command_to_contact(cntct,
                                                const_cast<char*>("mycommand")))
    throw(error() << "could not add default host notification command to "
                     "default contact");
  cntct->host_notification_commands->command_ptr = cmd;
  if (!add_service_notification_command_to_contact(
          cntct, const_cast<char*>("mycommand")))
    throw(error() << "could not add default service notification command to "
                     "default contact");
