...
int main(int argc, char* argv[]) {
  ...
  int c;
#ifdef HAVE_GETOPT_H
  while ((c = getopt_long(argc, argv, "+?h:M:s:H:S:c:e:m:", long_options,
                          &option_index)) != -1) {
#else
  ...
  if (mode == "benchmark") {
    ...
    additional.append("\n");
    engine_cfg cfg_files(additional, count, activehosts, activeservices,
                         passivehosts, passiveservices);
    std::cout << "Done\n";
    ...
    // Print results.
    std::cout << "\n"
              << "  Total passive check results                   " << count
              << "\n"
              ...
              << static_cast<double>(count) /
                     ((send_time != start_time) ? (send_time - start_time) : 1)
              << "\n"
              << "  Average check results processed per second    "
              << static_cast<double>(count) /
                     ((end_time != start_time) ? (end_time - start_time) : 1)
              << "\n";
  }
  // Generate configuration files.
    ...
    additional.append("\n");
    engine_cfg cfg_files(additional, count, activehosts, activeservices,
                         passivehosts, passiveservices, false);
    std::cout << "Configuration files generated in " << cfg_files.directory()
              << "\n";
  }
  ...
  // Print help.
    ...
    std::cout
        ...
        << "  -M --mode             This tool has multiple modes : "
           "'benchmark'\n"
        << "                        which compute the time needed to process "
           "some\n"
        << "                        count of passive check results, "
           "'configuration'\n"
        << "                        which generate configuration files "
           "compatible\n"
        << "                        with the latest mode 'commands' which "
           "print on\n"
        << "                        standard output passive check results.\n"
        << "  -h --activehosts      Number of active hosts in the "
           "configuration (default is "
        << activehosts << ").\n"
        << "  -s --activeservices   Number of active services in the "
           "configuration (default is "
        << activeservices << ").\n"
        << "  -H --passivehosts     Number of passive hosts in the "
           "configuration. (default is "
        << passivehosts << ").\n"
        << "  -S --passiveservices  Number of passive services in the "
           "configuration (default is "
        << passiveservices << ").\n"
        << "  -c --count            Number of passive check results to send "
           "(default is "
        << count << ")\n"
        << "Benchmark options\n"
        << "  -e --engine           Centreon Engine binary (default is "
        << engine << ")\n"
        << "  -m --module           Centreon Engine external command module "
           "(default is "
        << module << ")\n"
        << "\n"
