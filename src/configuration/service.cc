...
static unsigned int const   default_initial_state(engine::notifier::state_ok);
...
bool service::_set_initial_state(std::string const& value) {
  ...
  string::trim(data);
  if (data == "o" || data == "ok")
    _initial_state = notifier::state_ok;
  else if (data == "w" || data == "warning")
    _initial_state = notifier::state_warning;
  else if (data == "u" || data == "unknown")
    _initial_state = notifier::state_unknown;
  else if (data == "c" || data == "critical")
    _initial_state = notifier::state_critical;
  else
    return false;
  return true;
}
