...
int handle_async_service_check_result(
      com::centreon::engine::service2* temp_service,
  ...
  com::centreon::engine::service2* master_service = NULL;
  ...
  */
  ...
  if (temp_service->check_type == SERVICE_CHECK_PASSIVE) {
    if (config->log_passive_checks())
      logger(log_passive_check, basic)
        ...
        << temp_service->get_description() << ";" << temp_service->current_state
        << ";" << temp_service->plugin_output;
  }
  ...
  if (temp_service->current_state == STATE_OK) {
    ...
    if (hard_state_change) {
      ...
      service_other_props[{temp_service->host_ptr->get_name(),
    ...
    if (!service_other_props[{temp_service->host_ptr->get_name(),
                              temp_service->get_description()}]
             .recovery_been_sent &&
        !hard_state_change) {
      service_notification(
    ...
    if (service_other_props[std::make_pair(
          ...
          temp_service->get_description())].recovery_been_sent) {
      temp_service->current_notification_number = 0;
      ...
      service_other_props[std::make_pair(
            temp_service->host_ptr->get_name(),
            temp_service->get_description())].initial_notif_time = 0;
    }
    temp_service->problem_has_been_acknowledged = false;
    temp_service->acknowledgement_type = ACKNOWLEDGEMENT_NONE;
    temp_service->no_more_notifications = false;
    ...
    if (temp_service->current_attempt < temp_service->max_attempts) {
      ...
      if (config->enable_predictive_service_dependency_checks()
        ...
        std::pair<std::string, std::string>
          id(std::make_pair(temp_service->get_hostname(), temp_service->get_description()));
        ...
        for (umultimap<std::pair<std::string, std::string>,
          ...
          if (temp_dependency->dependent_service_ptr == temp_service
            ...
            master_service = (com::centreon::engine::service2*)temp_dependency->master_service_ptr;
            logger(dbg_checks, most)
  ...
  for (servicelist_item = check_servicelist;
    ...
    temp_service = (com::centreon::engine::service2*)servicelist_item->object_ptr;
 ...
 */
void schedule_service_check(com::centreon::engine::service2* svc, time_t check_time, int options) {
  logger(dbg_functions, basic)
    << "schedule_service_check()";
