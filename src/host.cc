 ...
 *          host is not found.
 */
host& com::centreon::engine::find_host(uint64_t host_id) {
 ...
 */
bool com::centreon::engine::is_host_exist(uint64_t host_id) throw() {
  host_id_map::const_iterator it(host::hosts_by_id.find(host_id));
...
int host::handle_async_check_result_3x(check_result* queued_check_result) {
  ...
  logger(dbg_checks, most)
      ...
      << logging::setprecision(3) << "\tExec Time:          " << execution_time
      << "\n"
...
void host::set_flap(double percent_change,
  ...
  logger(log_runtime_warning, basic)
      << logging::setprecision(1) << "HOST FLAPPING ALERT: " << get_name()
...
//          set_next_notification((time_t)(current_time + (60 * 60 * 24 *
//          365)));
