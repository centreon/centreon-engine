/*
** Copyright 2007-2008, 2015 Ethan Galstad
...
void display_scheduling_info() {
  ...
  logger(log_info_message, basic)
    ...
    << "Host check spread:                  " << scheduling_info.host_check_spread << " sec\n"
    << "First scheduled check:              "
    << ((scheduling_info.total_scheduled_hosts == 0)
        ? "N/A"
        : string::ctime(scheduling_info.first_host_check)) << "\n"
    << "Last scheduled check:               "
    << ((scheduling_info.total_scheduled_hosts == 0)
        ? "N/A"
        : string::ctime(scheduling_info.last_host_check)) << "\n"
    << "\n";
  ...
  logger(log_info_message, basic)
    ...
    << "Service inter-check delay:          " << scheduling_info.service_inter_check_delay << " sec\n"
    ...
    << "Service check spread:               " << scheduling_info.service_check_spread << " sec\n"
    << "First scheduled check:              " << string::ctime(scheduling_info.first_service_check) << "\n"
    << "Last scheduled check:               " << string::ctime(scheduling_info.last_service_check) << "\n"
    << "\n";
...
bool operator==(
  ...
  return (obj1.total_services == obj2.total_services
          ...
          && obj1.service_check_spread == obj2.service_check_spread
          && obj1.host_check_spread == obj2.host_check_spread
...
std::ostream& operator<<(std::ostream& os, sched_info const& obj) {
  os << "scheduling_info {\n"
    ...
    "  service_check_spread:                " << obj.service_check_spread << "\n"
    "  host_check_spread:                   " << obj.host_check_spread << "\n"
