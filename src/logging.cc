...
static void write_to_logs_and_console(char* buffer, unsigned long data_type, int display) {
  ...
  if (display == TRUE) {
    ...
    if (test_scheduling == TRUE
        && data_type == NSLOG_VERIFICATION_WARNING)
      return;
...
int log_host_states(unsigned int type, time_t* timestamp) {
  ...
  if (type == INITIAL_STATES
      && config.get_log_initial_states() == false)
    return (OK);
  ...
  for (temp_host = host_list;
    ...
    write_to_all_logs_with_timestamp(processed_buffer,
                                     NSLOG_INFO_MESSAGE,
				     timestamp);
...
int rotate_log_file(time_t rotation_time) {
  ...
  write_to_all_logs_with_timestamp(temp_buffer,
				   NSLOG_PROCESS_INFO,
                                   &rotation_time);
  delete[] temp_buffer;
...
int log_debug_info(int level, unsigned int verbosity, char const* fmt, ...) {
  ...
  if (!(config.get_debug_level() == DEBUGL_ALL
	|| (level & config.get_debug_level())))
    return (OK);
  ...
  fprintf(debug_file_fp, "[%lu.%06lu] [%03d.%d] [pid=%lu] ",
          current_time.tv_sec,
  ...
  if ((unsigned long)ftell(debug_file_fp) > config.get_max_debug_file_size()
      && config.get_max_debug_file_size() > 0L) {
