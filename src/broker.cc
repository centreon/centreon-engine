...
void broker_comment_data(int type, int flags, int attr,
                         int comment_type, int entry_type,
                         char const* host_name,
                         char const* svc_description, time_t entry_time,
                         char const* author_name, char* comment_data,
                         int persistent, int source, int expires,
...
void broker_downtime_data(int type, int flags, int attr,
                          int downtime_type, char* host_name,
                          char* svc_description, time_t entry_time,
                          char* author_name, char* comment_data,
                          time_t start_time, time_t end_time, int fixed,
...
void broker_program_status(int type,
  ...
  ds.active_service_checks_enabled = config.get_execute_service_checks();
  ds.passive_service_checks_enabled = config.get_accept_passive_service_checks();
  ds.active_host_checks_enabled = config.get_execute_host_checks();
  ds.passive_host_checks_enabled = config.get_accept_passive_host_checks();
  ds.event_handlers_enabled = config.get_enable_event_handlers();
  ds.flap_detection_enabled = config.get_enable_flap_detection();
  ds.failure_prediction_enabled = config.get_enable_failure_prediction();
  ...
  ds.global_host_event_handler = config.get_global_host_event_handler().toStdString().c_str();
  ds.global_service_event_handler = config.get_global_service_event_handler().toStdString().c_str();
...
int broker_notification_data(int type,
  ...
  /* make callbacks */
  return (neb_make_callbacks(NEBCALLBACK_NOTIFICATION_DATA, (void*)&ds));
}
...
int broker_contact_notification_data(int type,
  ...
  /* make callbacks */
  return (neb_make_callbacks(NEBCALLBACK_CONTACT_NOTIFICATION_DATA, (void*)&ds));
}
