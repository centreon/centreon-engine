 ...
 *          "ACTIVE".
 */
static std::string get_service_check_type(com::centreon::engine::service& svc,
                                          nagios_macros* mac) {
  (void)mac;
  return (checkable::check_passive == svc.get_check_type()) ? "PASSIVE"
                                                            : "ACTIVE";
}
 ...
 */
template <service::service_state((com::centreon::engine::service::*member)()
                                     const)>
static std::string get_service_state(com::centreon::engine::service& svc,
                                     nagios_macros* mac) {
  (void)mac;
  char const* state;
 ...
 */
static std::string get_service_macro_timezone(
...
struct grab_service_redirection {
  typedef std::unordered_map<
      unsigned int,
      std::pair<std::string (*)(com::centreon::engine::service&,
                                nagios_macros* mac),
                bool>>
      entry;
  entry routines{
      ...
      // State type.
      {MACRO_SERVICESTATETYPE, {&get_state_type<service>, true}},
      // State.
      {MACRO_SERVICESTATE,
      ...
      {MACRO_SERVICEEXECUTIONTIME,
       {&get_double<service, checkable, &checkable::get_execution_time, 3>,
        true}},
      // Latency.
      {MACRO_SERVICELATENCY,
       {&get_double<service, checkable, &checkable::get_latency, 3>, true}},
      // Last check.
      ...
      {MACRO_LASTSERVICECRITICAL,
       {&get_member_as_string<service,
                              time_t,
                              &service::get_last_time_critical>,
        true}},
      // Downtime.
      ...
      {MACRO_SERVICEPERCENTCHANGE,
       {&get_double<service,
                    checkable,
                    &checkable::get_percent_state_change,
                    2>,
        true}},
      // Duration.
      {MACRO_SERVICEDURATION, {&get_duration<service>, true}},
      // Duration in seconds.
      {MACRO_SERVICEDURATIONSEC, {&get_duration_sec<service>, true}},
      // Notification number.
