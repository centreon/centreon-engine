...
namespace          configuration {
  namespace        applier {
    ...
    protected:
      void         _diff(
                     std::list<std::shared_ptr<T> > const& old_objects,
                     std::list<std::shared_ptr<T> > const& new_objects) {
        difference<std::list<std::shared_ptr<T> > >
          diff(old_objects, new_objects);
        for (typename std::list<std::shared_ptr<T> >::const_iterator
      ...
      virtual void _add_object(std::shared_ptr<T> obj) = 0;
      virtual void _modify_object(std::shared_ptr<T> obj) = 0;
      virtual void _remove_object(std::shared_ptr<T> obj) = 0;
    };
