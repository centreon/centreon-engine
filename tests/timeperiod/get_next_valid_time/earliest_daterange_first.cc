...
// When get_next_valid_time() is called
// Then the next valid time is based on the offset weekday of generic month's timeranges
TEST_F(GetNextValidTimeEarliestDaterangeFirstTest, OffsetWeekdayOfGenericMonthFirst) {
  offset_weekday_of_generic_month_and_later();
  time_t computed((time_t)-1);
  get_next_valid_time(_now, &computed, _creator.get_timeperiods());
...
// When get_next_valid_time() is called
// Then the next valid time is based on the offset weekday of specific month's timeranges
TEST_F(GetNextValidTimeEarliestDaterangeFirstTest, OffsetWeekdayOfSpecificMonthFirst) {
  offset_weekday_of_specific_month_and_later();
  time_t computed((time_t)-1);
  get_next_valid_time(_now, &computed, _creator.get_timeperiods());
